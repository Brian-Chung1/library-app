...
function createBookElement(book) {
  ...
  editElement.addEventListener("click", () => {
    ...
    if (bookReadStatusElement.textContent === "")
...
function updateBookElement(currentBookElement, updatedBook) {
  ...
  currentBookElement.children[0].children[1].textContent = formatAuthor(
    updatedBook.author
  );
  currentBookElement.children[0].children[2].textContent = formatPages(
    updatedBook.pages
  );
...
function getUpdatedBook() {
  ...
  let updated = new Book(title.value, author.value, pages.value, readstatus);
  return updated;
}
...
function showAlert(message) {
  ...
  const alertLocation = document.getElementById("search-modal");
  alertLocation.insertAdjacentElement("afterend", alertMsg);
...
async function processData(searchTerms) {
  ...
  for (let i = 0; i < data.items.length; i++) {
    ...
    let author =
      data.items[i].volumeInfo.authors == void 0 ||
      typeof data.items[i].volumeInfo.authors == "undefined"
        ? "Unknown"
        : data.items[i].volumeInfo.authors[0];
    let pages =
      data.items[i].volumeInfo.pageCount == void 0 ||
      typeof data.items[i].volumeInfo.pageCount == "undefined"
        ? ""
